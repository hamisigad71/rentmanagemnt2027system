...
import { Search, Filter, Layers, LayoutGrid, CheckCircle2, AlertCircle, ArrowUpRight, MapPin, Building2, MoreHorizontal } from "lucide-react";
                    ...
                    >
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4 group/unit">
                           <div className="w-1.5 h-12 bg-slate-900 rounded-full group-hover/unit:bg-blue-600 transition-all duration-300" />
                           <div className="flex flex-col gap-1">
                              <span className="text-[14px] font-black text-slate-900 tracking-tighter leading-none">
                                 Unit {unit.number}
                              </span>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex flex-col gap-1">
                           <div className="flex items-center gap-1.5 font-black text-[11px] text-slate-900 uppercase tracking-widest leading-none underline decoration-blue-500/30 underline-offset-4 decoration-2">
                      ...
                      <td className="px-8 py-6">
                        <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">{unit.type}</span>
                      </td>
                      <td className="px-8 py-6">
                         <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded-lg bg-slate-50 border border-slate-100 font-black text-[10px] text-slate-500">
                            LEVEL {unit.floor}
                         </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                           <span className="text-sm font-black text-slate-900">KSh {unit.rent.toLocaleString()}</span>
                           <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Performance Monthly</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                         <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full font-black text-[9px] uppercase tracking-widest ${unit.status === 'occupied' ? 'bg-green-100 text-green-700 shadow-[0_4px_12px_rgba(34,197,94,0.15)]' : 'bg-orange-100 text-orange-700'}`}>
                            {unit.status}
                         </div>
                      </td>
          ...
          <div className="relative h-48 bg-slate-900 overflow-hidden">
             ...
             <div className="absolute inset-x-0 bottom-0 p-10 flex items-end justify-between z-10 bg-gradient-to-t from-slate-900 to-transparent">
                <div className="space-y-3">
                   ...
                   <h3 className="text-4xl font-black text-white tracking-tighter shrink-0 capitalize">Strategic <span className="text-blue-500 underline decoration-white/20 underline-offset-8">Unit {selectedUnit?.number}</span></h3>
                </div>
          ...
          <div className="p-10 space-y-10">
             ...
             <div className="space-y-6">
                ...
                <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white shadow-2xl shadow-slate-900/40 relative overflow-hidden group">
                   <div className="absolute top-0 right-0 w-32 h-32 bg-blue-600/20 blur-[60px] rounded-full group-hover:bg-blue-600/40 transition-all duration-1000" />
                   <div className="relative z-10 flex items-end justify-between">
                      <div className="space-y-1">
                         <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Scheduled Rent</p>
             ...
             <div className="flex gap-4 pt-10">
                <Button variant="premium" className="flex-1 h-16 rounded-2xl text-[11px] uppercase tracking-widest">Modify Digital Twin</Button>
                <Button variant="outline" className="flex-1 h-16 rounded-2xl text-[11px] uppercase tracking-widest">Status Protocol</Button>
             </div>
          </div>
        </div>
      </Modal>
